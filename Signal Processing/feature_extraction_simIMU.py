...
def fe_parallel(combo):
    ...
    base_path = "/HDD/hipexo/Inseung/sim IMU data/POSE" + str(folder) + "/"
    norm_path = "/HDD/hipexo/Inseung/norm_matrix/AB"+str(subject)+"_norm_IMU" + str(folder) +".csv"
    ...
    if path.exists(data_path) == 1:
        ...
        save_path_dir = "/HDD/hipexo/Inseung/sim IMU feature extraction data/"
        ...
        if trial_mode == "LG":
            save_path = save_path_dir+"AB"+str(subject)+"_"+str(trial_mode)+"_W"+str(window_size)+"_TP"+str(int(transition_point*100))+"_S"+str(walking_speed)+"_R"+str(trial_number)+"_IMU"+str(folder)+".csv"
            if starting_leg == "R":
                feature_extracted_data.to_csv(save_path, sep=',', index=False, header=False)
        else:
            save_path = save_path_dir+"AB"+str(subject)+"_"+str(trial_mode)+"_W"+str(window_size)+"_TP"+str(int(transition_point*100))+"_S2_"+str(starting_leg)+str(trial_number)+"_IMU"+str(folder)+".csv"
            if walking_speed == 2:
                feature_extracted_data.to_csv(save_path, sep=',', index=False, header=False)
                    ...
                    for walking_speed in [2]:
                        for trial_number in [1, 2, 3]:
                            for folder in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
                                run_combos.append([subject, window_size, trial_mode, starting_leg, transition_point, walking_speed, trial_number, folder])
